...
exports.getBoardOfDirectorById = async (req,res) =>{
    try{
        ...
        return successResponse(res, "Director data fetched successfully", boardOfDirector);
  ...
  try {
    ...
    validRows.forEach((row) => {
        ...
        mappedData.push({
            ...
            dsc: formatBoolean(row["Is DSC issued to the BoD?"]),
            dinIssued: formatBoolean(row["Is DIN issued to the BoD?"]),
            dinNumber: row["If yes, provide DIN no."] || null,
            farmerCert: formatBoolean(row["Farmer/ Producer Certificate collected?"]),
            isSanchalak: formatBoolean(row["Is Sanchalak/ Sahayyak (Yes/No)"] ),
